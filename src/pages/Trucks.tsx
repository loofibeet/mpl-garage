...
export function Trucks() {
  ...
  const load = useCallback(() => {
    const allTrucks    = db.getAll<Truck>('trucks').sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    const allCompanies = db.getAll<Company>('companies').sort((a, b) => a.name.localeCompare(b.name));
    setTrucks(allTrucks.map(tr => ({ ...tr, company: allCompanies.find(c => c.id === tr.company_id) })));
  ...
  const openEdit   = (truck: Truck) => {
    setEditItem(truck);
    setForm({ company_id: truck.company_id, make: truck.make, model: truck.model, year: String(truck.year || ''), plate_number: truck.plate_number, vin: truck.vin, mileage: String(truck.mileage), status: truck.status, color: truck.color, notes: truck.notes });
    setModalOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    const payload = { company_id: form.company_id, make: form.make, model: form.model, year: form.year ? parseInt(form.year) : null, plate_number: form.plate_number, vin: form.vin, mileage: parseInt(form.mileage) || 0, status: form.status, color: form.color, notes: form.notes };
    if (editItem) { db.update('trucks', editItem.id, payload); } else { db.insert('trucks', payload); }
  ...
  const filtered = trucks.filter(tr => {
    ...
    const matchSearch  = !q || tr.plate_number.toLowerCase().includes(q) || tr.vin.toLowerCase().includes(q) || tr.make.toLowerCase().includes(q) || tr.model.toLowerCase().includes(q) || tr.company?.name.toLowerCase().includes(q);
    ...
    <Layout title={t('trucks')}>
      <div className="space-y-5">
        ...
        {loading ? (
          ...
          <Card className="p-12 text-center"><TruckIcon className="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto mb-3" /><p className="text-slate-500 dark:text-slate-400">{t('noData')}</p><Button className="mt-4" icon={<Plus className="w-4 h-4" />} onClick={openCreate}>{t('addTruck')}</Button></Card>
        ) : (
              ...
              <Card key={truck.id} className="p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-100 dark:bg-slate-700 rounded-xl flex items-center justify-center flex-shrink-0"><TruckIcon className="w-5 h-5 text-slate-600 dark:text-slate-300" /></div>
                    <div><h3 className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{truck.make} {truck.model}</h3><p className="text-xs text-slate-500 dark:text-slate-400">{truck.company?.name}</p></div>
                  </div>
                  <Badge variant={statusVariant(truck.status)}>{statusLabel(truck.status)}</Badge>
                </div>
                <div className="grid grid-cols-2 gap-2 mt-3">
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400"><Hash className="w-3.5 h-3.5 text-orange-400" /><span className="font-mono font-semibold text-slate-700 dark:text-slate-300">{truck.plate_number}</span></div>
                  {truck.year && <div className="text-xs text-slate-500 dark:text-slate-400">{truck.year}</div>}
                  {truck.mileage > 0 && <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400"><Gauge className="w-3.5 h-3.5" />{truck.mileage.toLocaleString()} km</div>}
