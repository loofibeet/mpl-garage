...
import { db } from '../lib/storage';               // ← replaces supabase
...
export function Companies() {
  ...
  const load = () => {
    ...
    setCompanies(
      allCompanies.map(c => ({
        ...c,
        truck_count: allTrucks.filter(t => t.company_id === c.id).length,
      }))
    );
    setLoading(false);
  };
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    if (editItem) {
      db.update('companies', editItem.id, form);
    } else {
      db.insert('companies', form);
    }
    setSaving(false);
    setModalOpen(false);
    load();
  ...
  return (
    <Layout title={t('companies')}>
      <div className="space-y-5">
        ...
        {loading ? (
          ...
          <Card className="p-12 text-center">
            ...
            <Button className="mt-4" icon={<Plus className="w-4 h-4" />} onClick={openCreate}>{t('addCompany')}</Button>
          </Card>
        ) : (
              ...
              <Card key={company.id} className="p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-slate-800 dark:text-slate-100 text-sm leading-tight">{company.name}</h3>
                      {company.city && <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{company.city}</p>}
                    </div>
                  </div>
                  <Badge variant="info" className="flex-shrink-0">
                    <TruckIcon className="w-3 h-3 mr-1" />{company.truck_count || 0}
                  </Badge>
                </div>
                ...
                <div className="flex gap-2 pt-3 border-t border-slate-100 dark:border-slate-700/50">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-sky-600 dark:hover:text-sky-400 transition-colors px-2 py-1 rounded"
                  >
                    <Eye className="w-3.5 h-3.5" /> View
                  </button>
                  <button
                    onClick={() => openEdit(company)}
                    className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-orange-600 dark:hover:text-orange-400 transition-colors px-2 py-1 rounded"
                  >
                    <Pencil className="w-3.5 h-3.5" /> {t('edit')}
                  </button>
                  <button
                    onClick={() => handleDelete(company.id)}
                    className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-red-600 dark:hover:text-red-400 transition-colors px-2 py-1 rounded ml-auto"
                  >
      ...
      <Modal
        ...
        title={editItem ? `Edit: ${editItem.name}` : t('addCompany')}
      >
