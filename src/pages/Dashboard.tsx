...
interface DashboardStats {
  ...
  totalRevenue: number;
  recentJobs: (RepairJob & { company?: Company; truck?: Truck })[];
  todayJobs: number;
}
...
export function Dashboard() {
  ...
  const [stats, setStats] = useState<DashboardStats>({
    totalJobs: 0, activeJobs: 0, pendingInvoices: 0, totalRevenue: 0, recentJobs: [], todayJobs: 0,
  });
  ...
  useEffect(() => {
    ...
    const recentJobs = [...jobs]
      ...
      .map(job => ({
        ...
        truck:   trucks.find(t => t.id === job.truck_id),
      }));
    ...
    setStats({
      ...
      todayJobs:       jobs.filter(j => j.start_date === today).length,
      recentJobs,
    });
    setLoading(false);
  }, []);
  ...
  const statusLabel   = (s: string) => {
    const map: Record<string, string> = {
      pending: t('pending'), in_progress: t('inProgress'),
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard
              ...
              icon={<ClipboardList className="w-5 h-5 text-sky-600 dark:text-sky-400" />}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: <Clock className="w-4 h-4" />,       label: "Today's Jobs", value: stats.todayJobs,  color: 'text-blue-600 dark:text-blue-400',   bg: 'bg-blue-50 dark:bg-blue-900/20' },
              { icon: <AlertCircle className="w-4 h-4" />, label: 'In Progress',  value: stats.activeJobs, color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-900/20' },
              { icon: <TruckIcon className="w-4 h-4" />,   label: 'Pending Jobs', value: stats.recentJobs.filter(j => j.status === 'pending').length, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/20' },
            ].map(item => (
          ...
          <Card>
            <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
              ...
              <a href="/jobs" className="text-sm text-orange-500 hover:text-orange-600 font-medium">View all</a>
            </div>
                ...
                {stats.recentJobs.map(job => (
                  ...
                  >
                    <div className="w-10 h-10 bg-slate-100 dark:bg-slate-700 rounded-xl flex items-center justify-center flex-shrink-0">
                      <TruckIcon className="w-5 h-5 text-slate-500 dark:text-slate-400" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="font-medium text-slate-800 dark:text-slate-200 text-sm truncate">{job.job_number}</p>
                        <Badge variant={priorityVariant(job.priority)} className="flex-shrink-0">{priorityLabel(job.priority)}</Badge>
                      </div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                        {job.company?.name || '—'} · {job.truck?.make} {job.truck?.model} · {job.truck?.plate_number}
                      </p>
                    </div>
