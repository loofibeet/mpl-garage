...
export function Workers() {
  ...
  const handleAddWorker = () => {
    ...
    try { db.insert('workers', payload); } catch (err) { console.error("Erreur insertion ouvrier :", err); }
  ...
  return (
    <Layout title={`${capitalisedMois} ${year} - Planning des Ouvriers`}>
      <div className="space-y-6">
        ...
        {loading ? (
          <div className="flex items-center justify-center h-48 text-slate-400"><div className="w-6 h-6 border-2 border-orange-500 border-t-transparent rounded-full animate-spin mr-3" />Chargement de la matrice de données...</div>
        ) : workers.length === 0 ? (
          <Card className="p-12 text-center"><p className="text-slate-500 mb-4">Aucun ouvrier enregistré dans la base de données pour le moment.</p><Button icon={<Plus className="w-4 h-4" />} onClick={() => setModalOpen(true)}>Créer le premier profil</Button></Card>
        ) : (
          <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-900">
            <table className="w-full border-collapse text-left text-sm">
              <thead className="bg-slate-50 dark:bg-slate-800/70 sticky top-0 border-b border-slate-200 dark:border-slate-700 z-10">
                <tr>
                  <th className="p-3 font-semibold text-slate-700 dark:text-slate-300 min-w-[120px] bg-slate-100 dark:bg-slate-800 sticky left-0 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Jours / Équipe</th>
                  {workers.map(w => (
                    <th key={w.id} className="p-3 font-bold text-center text-slate-800 dark:text-slate-200 min-w-[160px] border-l border-slate-200 dark:border-slate-700 relative z-20">
                      <div className="flex items-center justify-between gap-1 w-full px-1">
                        ...
                        <div className="relative">
                          <button onClick={(e) => { e.stopPropagation(); setActiveMenuId(activeMenuId === w.id ? null : w.id); }} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-all" title="Options"><MoreVertical className="w-4 h-4" /></button>
                          {activeMenuId === w.id && (
                            <div className="absolute right-0 mt-1 w-40 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl py-1 z-30 text-left">
                              <button onClick={(e) => { e.stopPropagation(); handleRemoveWorker(w.id, w.name); setActiveMenuId(null); }} className="flex items-center gap-2 w-full px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"><Trash2 className="w-3.5 h-3.5" /> Supprimer le profil</button>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  ...
                  return (
                    <tr key={day} className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors ${isToday ? 'bg-orange-50/40 dark:bg-orange-500/5' : ''}`}>
                      <td className={`p-2.5 font-mono font-bold sticky left-0 z-10 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] ${isToday ? 'bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-400' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400'}`}>
                        <div className="flex items-center justify-center gap-1"><span>Jour {String(day).padStart(2, '0')}</span>{isToday && <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" />}</div>
                      </td>
                      {workers.map(w => {
                        ...
                        return (
                          <td key={w.id} onClick={() => setDetailPanel({ worker: w, day })} className="p-2 border-l border-slate-100 dark:border-slate-800 text-center cursor-pointer hover:bg-orange-500/10 dark:hover:bg-orange-500/20 transition-all select-none">
                            {isAbsent ? (
                              <div className="inline-flex items-center justify-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-red-50 text-red-600 dark:bg-red-950/50 dark:text-red-400 border border-red-200/50 dark:border-red-900/50 w-full"><XCircle className="w-3.5 h-3.5" /> Absent</div>
                            ) : (
                              <div className="inline-flex flex-col items-center justify-center p-1 rounded-lg w-full bg-slate-50/60 dark:bg-slate-800/40 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 text-xs">
                                ...
                                {(dayData.tips > 0 || dayData.lunchMoney) && <div className="text-[10px] text-slate-400 mt-0.5 flex gap-1">{dayData.tips > 0 && <span>+{dayData.tips}€ P.</span>}{dayData.lunchMoney && <span title="Repas Inclus">🍴</span>}</div>}
                              </div>
                            )}
                          </td>
                        );
                ...
                <tr className="bg-slate-100 dark:bg-slate-800 font-semibold border-t-2 border-slate-300 dark:border-slate-600 shadow-inner">
                  <td className="p-4 sticky left-0 bg-slate-200 dark:bg-slate-700 z-10 font-bold text-slate-700 dark:text-slate-300 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Totaux du Mois</td>
                  {workers.map(w => {
                    ...
                    return (
                      <td key={w.id} className="p-3 border-l border-slate-200 dark:border-slate-600 text-xs text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/90">
                        <div className="space-y-1 font-mono">
                          <div className="flex justify-between border-b border-slate-200/60 dark:border-slate-700 pb-0.5"><span>Heures :</span><span className="font-bold text-slate-900 dark:text-slate-100">{totals.totalHours} h</span></div>
                          <div className="flex justify-between text-red-600 dark:text-red-400"><span>Absences :</span><span>{totals.absentDays} j</span></div>
                          <div className="flex justify-between text-amber-600 dark:text-amber-400"><span>Primes/Repas :</span><span>{totals.totalTips + totals.totalLunch}€</span></div>
                          <div className="flex justify-between text-sm pt-1 font-bold text-emerald-600 dark:text-emerald-400 border-t border-dashed border-slate-300 dark:border-slate-600"><span className="flex items-center gap-0.5"><Landmark className="w-3 h-3" /> Net à Payer :</span><span>{totals.totalEarnings.toFixed(2)}€</span></div>
                        </div>
                      </td>
                    );
                  })}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Enregistrer un nouvel ouvrier">
        <div className="space-y-4">
          <Input label="Nom complet de l'ouvrier *" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Ex: Jean Dupont" />
          <Input label="Tarif horaire de base (€)" type="number" value={form.hourly_rate} onChange={e => setForm(f => ({ ...f, hourly_rate: e.target.value }))} placeholder="20" />
      ...
      <Modal open={!!detailPanel} onClose={() => setDetailPanel(null)} title={detailPanel ? `${detailPanel.worker.name} — Jour ${String(detailPanel.day).padStart(2, '0')}` : ''}>
        {detailPanel && (() => {
          const currentDayLog = getDayData(detailPanel.worker.id, detailPanel.day);
          return (
            <div className="space-y-4">
              <Select label="Statut de présence" value={currentDayLog.status} onChange={e => updateDayData(detailPanel.worker.id, detailPanel.day, { status: e.target.value as 'present' | 'absent' })} options={[{ value: 'present', label: '🟢 Présent & Actif' }, { value: 'absent', label: '🔴 Absent confirmé' }]} />
              {currentDayLog.status === 'present' && (
                <div className="space-y-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
                  <div className="grid grid-cols-2 gap-3">
                    <Input label="Heures effectuées" type="number" step="0.5" value={currentDayLog.hours} onChange={e => updateDayData(detailPanel.worker.id, detailPanel.day, { hours: parseFloat(e.target.value) || 0 })} />
                    <Input label="Primes / Pourboires (€)" type="number" value={currentDayLog.tips} onChange={e => updateDayData(detailPanel.worker.id, detailPanel.day, { tips: parseFloat(e.target.value) || 0 })} />
                  </div>
                  <Input label="Modifier le taux horaire (€) - Optionnel" placeholder={`Tarif de base actuel : ${detailPanel.worker.hourly_rate}€`} type="number" value={currentDayLog.rateOverride || ''} onChange={e => updateDayData(detailPanel.worker.id, detailPanel.day, { rateOverride: e.target.value ? parseFloat(e.target.value) : undefined })} />
                  <label className="flex items-center gap-3 p-2 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer select-none">
                    ...
                    <div className="text-xs"><p className="font-semibold text-slate-800 dark:text-slate-200">Inclure l'indemnité repas du jour</p><p className="text-slate-400">Ajoute une prime forfaitaire de +15.00€</p></div>
                  </label>
                </div>
              )}
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 flex items-center gap-1"><FileText className="w-3.5 h-3.5" /> Notes de suivi & Commentaires</label>
                <Textarea value={currentDayLog.notes} onChange={e => updateDayData(detailPanel.worker.id, detailPanel.day, { notes: e.target.value })} rows={3} placeholder="Exemple : Arrivé en retard, problèmes logistiques..." />
              </div>
              <div className="pt-2"><Button className="w-full justify-center" onClick={() => setDetailPanel(null)}>Valider et mettre à jour le tableau</Button></div>
            </div>
          );
        })()}
      </Modal>
